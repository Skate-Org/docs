...
To understand it further, we go over the following two scenarios.
...
All incentives earned by the vault are given back to the users. The ARB and VRTX incentives earned by the Arbitrum vault are claimable by the users on the vault’s UI and are updated at the end of each rewards respective epoch, while the Blast Points and Blast Gold earned by the Blast vault are transferred to the users after the epoch ends.
