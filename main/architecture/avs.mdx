...
Preconfirmation AVS verifies all execution calldata submitted by executors, which is published to the [Message Box contract](/main/architecture/kernel#kernel-components). 
...
```mermaid
  flowchart 
    ...
    attester3 -- "subscribe" --> p2p
    p2p -. "task" .-> attester3
    
    attester1 -- "vote" --> blsAggregator
    attester2 -- "vote" --> blsAggregator
    attester3 -- "vote" --> blsAggregator

    quorumReached{"Is Quorum\n Reached?"}
    blsAggregator -- "2) check" --> quorumReached
